...
def default_ledger() -> Path:
    return Path.home() / "Library/Application Support/Shakespeare/personalization/training_events.jsonl"


def default_registry() -> Path:
    return Path.home() / "Library/Application Support/Shakespeare/personalization/model_registry.json"
...
def compile_command(args: argparse.Namespace) -> int:
    ...
    if result.sft_train_examples == 0:
        print("No SFT training examples were produced; collect more accepted edits or longer document snapshots.")
    return 0
...
def _final_checkpoint(log_path: Path) -> dict[str, object]:
    ...
    records = [json.loads(line) for line in checkpoint_file.read_text(encoding="utf-8").splitlines() if line]
    finals = [record for record in records if record.get("final") is True and record.get("sampler_path")]
    if not finals:
...
async def _train(args: argparse.Namespace) -> None:
    ...
    if not train_file.exists() or not manifest.exists():
        raise RuntimeError("Compile the ledger before training; sft_train.jsonl and manifest.json are required")
    if train_file.stat().st_size == 0:
        raise RuntimeError("sft_train.jsonl is empty")

    try:
        from tinker_cookbook import model_info
        from tinker_cookbook.supervised import FromConversationFileBuilder, ChatDatasetBuilderCommonConfig
        from tinker_cookbook.supervised import train
    except ImportError as error:
        raise RuntimeError("Install Trainer dependencies with: python -m pip install -e Trainer") from error
    ...
    if args.promote:
        _promote_checkpoint(args.registry, checkpoint, args.base_model, manifest)
...
def dpo_command(args: argparse.Namespace) -> int:
    ...
    if not train_file.exists() or train_file.stat().st_size == 0:
        raise RuntimeError("dpo_train.jsonl is missing or empty; collect rejected edit decisions first")
    ...
    comparisons = ComparisonBuilderFromJsonl(
        ...
        test_path=str(eval_file) if eval_file.exists() and eval_file.stat().st_size else None,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compile and train Shakespeare personal style models")
    subparsers = parser.add_subparsers(dest="command", required=True)

    compile_parser = subparsers.add_parser("compile", help="Compile the local event ledger")
    compile_parser.add_argument("--input", type=Path, default=default_ledger())
    compile_parser.add_argument("--output", type=Path, default=Path("Trainer/data/compiled"))
    ...
    train_parser.add_argument("--dataset-dir", type=Path, default=Path("Trainer/data/compiled"))
    ...
    train_parser.add_argument("--promote", action="store_true")
    train_parser.add_argument("--registry", type=Path, default=default_registry())
    train_parser.set_defaults(handler=train_command)

    dpo_parser = subparsers.add_parser("train-dpo", help="Run preference training from rejected edits")
    dpo_parser.add_argument("--dataset-dir", type=Path, default=Path("Trainer/data/compiled"))
    ...
    dpo_parser.add_argument("--load-checkpoint", help="Optional SFT state_path to continue from")
    ...
    dpo_parser.add_argument("--promote", action="store_true")
    dpo_parser.add_argument("--registry", type=Path, default=default_registry())
